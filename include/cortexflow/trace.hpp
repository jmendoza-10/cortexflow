...
#ifndef CORTEXFLOW_TRACE_LEVEL_VALUE
#define CORTEXFLOW_TRACE_LEVEL_VALUE 4
#endif

inline constexpr TraceLevel kTraceLevel =
    static_cast<TraceLevel>(CORTEXFLOW_TRACE_LEVEL_VALUE);
...
#define CORTEXFLOW_TRACE(lvl, kind, from, to, type_name, message)        \
    do {                                                                 \
        if constexpr (::cortexflow::TraceLevel::lvl <=                   \
                      ::cortexflow::kTraceLevel) {                       \
            ::cortexflow::detail::trace_emit(                            \
                ::cortexflow::TraceLevel::lvl,                           \
                (kind), (from), (to), (type_name), (message));          \
        }                                                                \
    } while (0)

#define CORTEXFLOW_TRACE_ERROR(kind, from, to, type_name, msg)    \
    CORTEXFLOW_TRACE(Error, kind, from, to, type_name, msg)
#define CORTEXFLOW_TRACE_WARN(kind, from, to, type_name, msg)     \
    CORTEXFLOW_TRACE(Warn, kind, from, to, type_name, msg)
#define CORTEXFLOW_TRACE_INFO(kind, from, to, type_name, msg)     \
    CORTEXFLOW_TRACE(Info, kind, from, to, type_name, msg)
#define CORTEXFLOW_TRACE_DISPATCH(kind, from, to, type_name, msg) \
    CORTEXFLOW_TRACE(Dispatch, kind, from, to, type_name, msg)
#define CORTEXFLOW_TRACE_FULL(kind, from, to, type_name, msg)     \
    CORTEXFLOW_TRACE(Full, kind, from, to, type_name, msg)

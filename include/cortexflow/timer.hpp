...
public:
    ...
    [[nodiscard]] Timer arm(Clock::duration delay, Envelope env) {
        ...
        heap_.push_back(Entry{due, seq, std::move(env)});
    ...
    void fire_due() {
        ...
        void* ctx = nullptr;
        {
            ...
            while (!heap_.empty() && heap_.front().due <= now) {
                ...
                live_seqs_.erase(it);
                to_fire.push_back(std::move(entry.envelope));
            }
        }
        if (to_fire.empty()) {
            return;
...
private:
    ...
    void cancel_by_seq(std::size_t seq) noexcept {
        std::lock_guard<std::mutex> lock(mutex_);
        // Lazy cancel: removing the seq from `live_seqs_` is the cancellation
        // signal — `fire_due` skips any popped entry whose seq is not in the
        // set. Calling cancel after the timer fired (when the seq is no
        // longer live) is a no-op, which keeps Timer destruction safe even
        // post-expiry.
        live_seqs_.erase(seq);
    }

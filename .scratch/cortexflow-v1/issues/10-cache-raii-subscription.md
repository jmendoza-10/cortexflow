...
The full subscription lifecycle. `Subscription` is a RAII handle returned by `cache.subscribe<K>(subscriber_id)`; its destructor releases the slot. The subscription pool size is the `MaxSubscriptions<N>` declared in `Config`; pool overflow is a `FRAMEWORK_ASSERT` (system failure, not a runtime error).
...
- [ ] Overflow on `subscribe` triggers `FRAMEWORK_ASSERT` naming the cap and the key

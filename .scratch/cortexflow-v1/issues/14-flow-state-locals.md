...
Each state function declares a typed state-locals struct (e.g., via a `using Locals = …;` member of a state-tag type). The framework allocates a single compile-time-sized buffer in the `Flow<S>` instance, sized to the largest `Locals` across the flow (computed at compile time via `std::max({sizeof(Ts::Locals)...})` over the state list). There is **no runtime allocation per transition**.

On entry to a state, the framework constructs that state's `Locals` in place inside the buffer. On transition, it destructs the outgoing `Locals` and constructs the incoming. RAII members of `Locals` (subscriptions, timers, anything held by value) follow this lifetime automatically — no manual cleanup in state functions.
